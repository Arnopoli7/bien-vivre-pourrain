...
const COMMISSION_ID = "infos-elus"
...
export default function InfosElusPage() {
  const { currentUser, documents, mettreALaCorbeille, peutSupprimer } = useApp()
  ...
  const docsInfos = documents.filter(d => d.commissionId === COMMISSION_ID)
  const docsAnnee = docsInfos.filter(d => d.annee === selectedYear)
  ...
  function handleCorbeilleAnnee(annee: number) {
    const docs = docsInfos.filter(d => d.annee === annee)
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm text-gray-500 mb-1">Espace partagé</p>
          <h1 className="text-2xl font-bold text-[#B4432E]">📌 Infos Élus</h1>
          <p className="text-sm text-gray-400 mt-1">
            Informations internes, notes de service et documents réservés aux élus
          </p>
        </div>
        {canWrite && (
          <button
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {YEARS.map(year => {
          const n = docsInfos.filter(d => d.annee === year).length
          return (
