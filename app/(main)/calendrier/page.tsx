...
// ── MODAL MODIFICATION RÉUNION ────────────────────────────────────────────────

function ModifierReunionModal({
  reunion,
  onClose,
}: {
  reunion: Reunion
  ...
  const { commissions, modifierReunion } = useApp()
  const [date, setDate] = useState(reunion.date)
  const [commissionId, setCommissionId] = useState(reunion.commissionId)
  const [titre, setTitre] = useState(reunion.titre ?? "")
  const [heure, setHeure] = useState(reunion.heure)
  const [lieu, setLieu] = useState(reunion.lieu)
  const [ordresDuJour, setOrdresDuJour] = useState<string[]>(reunion.ordresDuJour ?? [])
  const [nouveauPoint, setNouveauPoint] = useState("")
  const [erreur, setErreur] = useState("")

  function ajouterPoint() {
    if (!nouveauPoint.trim()) return
    setOrdresDuJour(prev => [...prev, nouveauPoint.trim()])
    setNouveauPoint("")
  }

  function supprimerPoint(index: number) {
    setOrdresDuJour(prev => prev.filter((_, i) => i !== index))
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const updated: Reunion = {
      ...
      lieu: lieu.trim(),
      titre: titre.trim(),
      ordresDuJour: ordresDuJour.length > 0 ? ordresDuJour : undefined,
    }
    modifierReunion(updated)
    const commission = commissions.find(c => c.id === commissionId)
    fetch("/api/notify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        type: "reunion_modifiee",
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
          <h2 className="text-lg font-semibold text-[#1A1A1A]">Modifier la réunion</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto px-6 pb-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              Titre ou objet <span className="text-[#B4432E]">*</span>
            </label>
            <input
              type="text"
              value={titre}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              Lieu <span className="text-[#B4432E]">*</span>
            </label>
            <input
              type="text"
              value={lieu}
          ...
          <div>
              ...
              <ol className="mb-2 space-y-1">
                {ordresDuJour.map((point, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-gray-700 bg-gray-50 rounded-lg px-3 py-1.5">
                    <span className="text-gray-400 font-semibold shrink-0">{i + 1}.</span>
                    <span className="flex-1">{point}</span>
                    <button type="button" onClick={() => supprimerPoint(i)} className="text-gray-400 hover:text-red-500 transition-colors shrink-0">✕</button>
                  </li>
                ))}
              </ol>
            )}
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); ajouterPoint() } }}
              ...
              <button type="button" onClick={ajouterPoint}
                className="px-3 py-2 text-sm font-medium text-white bg-[#B4432E] hover:bg-[#8B3222] rounded-xl transition-colors shrink-0">
                + Ajouter
              </button>
            </div>
          </div>
...
function ReunionDetailModal({
  ...
  const elus = useElus()
  const canEdit = currentUser?.role === "maire" || currentUser?.role === "adjoint" || currentUser?.role === "redacteur"
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-6 pb-5 border-b border-gray-100">
          ...
          {reunion.ordresDuJour && reunion.ordresDuJour.length > 0 && (
            <div className="mt-3 bg-gray-50 rounded-xl px-4 py-3">
              <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Ordre du jour</p>
              <ol className="space-y-1">
                {reunion.ordresDuJour.map((point, i) => (
          ...
          {canEdit ? (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-sm font-medium text-blue-600 hover:text-blue-800 border border-blue-200 hover:bg-blue-50 px-4 py-2 rounded-xl transition-colors"
              >
                ✏️ Modifier
              </button>
            </div>
...
function AjouterReunionModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const r: Reunion = {
      ...
      lieu: lieu.trim(),
      titre: titre.trim(),
      ...(ordresDuJour.length > 0 && { ordresDuJour }),
    }
    ajouterReunion(r)
...
export default function CalendrierPage() {
  ...
  const elus = useElus()
  const canEditReunions = currentUser?.role === "maire" || currentUser?.role === "adjoint" || currentUser?.role === "redacteur"
