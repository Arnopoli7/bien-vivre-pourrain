...
import { supprimerDocumentsParCompteRenduId } from "@/lib/firebase/firestore"
...
export default function NotesPage() {
  ...
  // ID de la commission "Réunion Maire et Adjoints" (accès restreint)
  ...
  // Brouillons : maire → tous | adjoint → ses propres | suppléant → ses propres + commission | rédacteur → ses propres | membre → commission
  const brouillons = crs.filter(c => {
    ...
    // Suppléant : voit ses propres brouillons + brouillons en lecture seule de sa commission suppléante
  ...
  // En attente : même logique que brouillons
  const enAttente = crs.filter(c => {
    if (c.statut !== "en_attente") return false
    if (isMaire) return true
    if (isAdjoint) return c.redacteur === userNom
    if (isRedacteur) return c.redacteur === userNom
    // Suppléant : voit ses propres CRs en attente
    if (getSuppleantsCommission(c.commissionId).includes(userNom)) return c.redacteur === userNom
    return getMembresCommission(c.commissionId).includes(userNom)
  })

  // Validés : tous les élus, sauf "Réunion Maire et Adjoints" restreint aux 4 membres
  const valides = crs.filter(c => {
    if (c.statut !== "valide") return false
    if (c.commissionId === REUNION_MA_ID) return MEMBRES_REUNION_MA.includes(userNom)
    return true
  })
  const totalVisible = brouillons.length + enAttente.length + valides.length
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {totalVisible} compte{totalVisible !== 1 ? "s" : ""} rendu{totalVisible !== 1 ? "s" : ""}
          </p>
        </div>
        {canWrite && (
          <Link
            ...
            <table className="w-full">
              ...
              <tbody>
                {brouillons.map(cr => {
                  ...
                  return (
                    <CRRow key={cr.id} cr={cr} commissions={commissions} currentUser={currentUser} onSupprimer={canDeleteThis ? handleSupprimer : undefined} />
                  )
                })}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-blue-100 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {enAttente.map(cr => (
                  <CRRow key={cr.id} cr={cr} commissions={commissions} currentUser={currentUser} onSupprimer={currentUser?.role === "maire" ? handleSupprimer : undefined} />
                ))}
              </tbody>
            </table>
          </div>
